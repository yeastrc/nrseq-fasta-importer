 ...
 *
 */
public class TaxonomyIdFrom_BracketSpecies_inDescription implements TaxonomyIdLookupIF {

	private static final Logger log = Logger.getLogger(TaxonomyIdFrom_BracketSpecies_inDescription.class);
	...
	private TaxonomyIdFrom_BracketSpecies_inDescription() { }
	public static TaxonomyIdFrom_BracketSpecies_inDescription getInstance() { 
		return new TaxonomyIdFrom_BracketSpecies_inDescription(); 
	}
		...
		if ( fullHeaderString == null ) {
			...
			if (gm.matches()) {
				...
				try {
					...
					String msg = "Error processing FASTA header that contains species name surrounded by '[]'."
